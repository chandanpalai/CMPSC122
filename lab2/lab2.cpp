...
//PRE:  0 < numofCategories <= MAX_CATEGORIES is initialized.
...
//		 categories. For i from 0 to numofCategories-1, the (i+1)th interval has
//		 lowerBound[i] as lower bound and upperBound[i] as upper bound. They form parallel 
//		 arrays. The histogram is displayed in a way that each interval is listed in a 
...
//In this method, the range of every category is established. All the elements in the input 
//data are allocated to categories according to the element value relative to the category 
//range. The displayHisrogram method is called and a histogram will be generated.
{
	//DATA DICTIONARY
	...
	for (int i = 0; i < numofCategories; i++)				
	{
		...
		numofElements[i] = 0;								//Initial the number of elements in 
